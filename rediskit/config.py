import os

from DockerBuildSystem import TerminalTools

from rediskit.utils import base64JsonToDict

TerminalTools.LoadDefaultEnvironmentVariablesFile("private.env")
TerminalTools.LoadDefaultEnvironmentVariablesFile(".env")

# Redis Settings
REDIS_HOST = os.environ.get("REDIS_HOST", "localhost")
REDIS_PORT = int(os.environ.get("REDIS_PORT", "6379"))
REDIS_PASSWORD = os.environ.get("REDIS_PASSWORD", "")
REDIS_TOP_NODE = os.environ.get("REDIS_TOP_NODE", "redis_kit_node")
REDIS_SCAN_COUNT = int(os.environ.get("REDIS_SCAN_COUNT", "10000"))
REDIS_SKIP_CACHING = os.environ.get("REDIS_SKIP_CACHING", "false").upper() == "TRUE"

# Lock Settings
REDIS_KIT_LOCK_SETTINGS_REDIS_NAMESPACE = os.environ.get("REDIS_KIT_LOCK_SETTINGS_REDIS_NAMESPACE", f"{REDIS_TOP_NODE}:LOCK")
REDIS_KIT_LOCK_ASYNC_SETTINGS_REDIS_NAMESPACE = os.environ.get("REDIS_KIT_LOCK_ASYNC_SETTINGS_REDIS_NAMESPACE", f"{REDIS_TOP_NODE}:LOCK_ASYNC")
REDIS_KIT_LOCK_CACHE_MUTEX = os.environ.get("REDIS_KIT_LOCK_CACHE_MUTEX", "REDIS_KIT_LOCK_CACHE_MUTEX")

REDIS_KIT_ENCRYPTION_SECRET = base64JsonToDict(os.environ.get("REDIS_KIT_ENCRYPTION_SECRET", ""))

# Semaphore Settings
REDIS_KIT_SEMAPHORE_SETTINGS_REDIS_NAMESPACE = os.environ.get("REDIS_KIT_SEMAPHORE_SETTINGS_REDIS_NAMESPACE", f"{REDIS_TOP_NODE}:Semaphore")
REDIS_KIT_SEMAPHORE_SETTINGS_STALE_TIMEOUT_SECONDS = int(os.environ.get("REDIS_KIT_SEMAPHORE_SETTINGS_STALE_TIMEOUT_SECONDS", "30"))
REDIS_KIT_SEMAPHORE_LOCK_TIME_TO_LIVE = int(os.environ.get("REDIS_KIT_SEMAPHORE_LOCK_TIME_TO_LIVE", "30"))
REDIS_KIT_SEMAPHORE_SETTINGS_ANALYZE_RAW_REVIEW_QUERY_SEMAPHORE_COUNT = int(os.environ.get("REDIS_KIT_SEMAPHORE_SETTINGS_ANALYZE_RAW_REVIEW_QUERY_SEMAPHORE_COUNT", "2"))
REDIS_KIT_SEMAPHORE_SETTINGS_TRANSLATE_TEXT_QUERY_SEMAPHORE_COUNT = int(os.environ.get("REDIS_KIT_SEMAPHORE_SETTINGS_TRANSLATE_TEXT_QUERY_SEMAPHORE_COUNT", "20"))
REDIS_KIT_SEMAPHORE_SETTINGS_SURVEY_BUILDER_QUERY_SEMAPHORE_COUNT = int(os.environ.get("REDIS_KIT_SEMAPHORE_SETTINGS_SURVEY_BUILDER_QUERY_SEMAPHORE_COUNT", "20"))
REDIS_KIT_SEMAPHORE_SETTINGS_LOOKUP_QUERY_SEMAPHORE_COUNT = int(os.environ.get("REDIS_KIT_SEMAPHORE_SETTINGS_LOOKUP_QUERY_SEMAPHORE_COUNT", "150"))
REDIS_KIT_SEMAPHORE_SETTINGS_WEXTRACTOR_QUERY_SEMAPHORE_COUNT = int(os.environ.get("REDIS_KIT_SEMAPHORE_SETTINGS_WEXTRACTOR_QUERY_SEMAPHORE_COUNT", "1"))
REDIS_KIT_SEMAPHORE_SETTINGS_GENERATE_REPLIES_QUERY_SEMAPHORE_COUNT = int(os.environ.get("REDIS_KIT_SEMAPHORE_SETTINGS_GENERATE_REPLIES_QUERY_SEMAPHORE_COUNT", "30"))
