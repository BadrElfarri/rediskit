#!/usr/bin/env python3
"""Test script to verify the public API works correctly."""
...
def test_custom_connection():
    ...
    @RedisMemoize(
        memoizeKey="test_custom", 
        ttl=300,
        connection=custom_redis
    )
...
def test_hash_storage():
    ...
    @RedisMemoize(
        memoizeKey=lambda tenantId, user_id: f"user:{tenantId}:{user_id}",
        ttl=600,
        storageType="hash",
